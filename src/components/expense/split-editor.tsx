...
export function SplitParticipants({
  ...
  return (
    <div className="space-y-2">
      ...
      {needsValue ? (
        // Exact / percent / shares need an input per member, so they keep the
        // list layout with an aligned value column.
        ...
        // Equal / personal need no input, so the members sit as compact chips.
        <div className="flex flex-wrap gap-2">
          {members.map((member) => {
            ...
            return (
              <label
                key={member.id}
                className={`inline-flex min-h-11 max-w-full cursor-pointer items-center gap-2 rounded-lg border px-2.5 transition-colors ${
                  selected
              ...
              >
                <input
                  type={method === 'personal' ? 'radio' : 'checkbox'}
                  name={method === 'personal' ? 'personal-member' : undefined}
                  checked={selected}
                  onChange={() =>
                    method === 'personal' ? onSelectOnly(member.id) : onToggleParticipant(member.id)
                  }
                  className="size-4 shrink-0 accent-brand focus:outline-2 focus:outline-offset-2 focus:outline-brand"
                />
                <MemberAvatar name={member.displayName} avatarUrl={member.avatarUrl} size="sm" />
                <span
                  className={`min-w-0 truncate text-sm ${selected ? 'font-medium text-ink' : 'text-muted'}`}
                >
                  {member.displayName}
                </span>
                {selected && amount !== undefined && totalMinor > 0 ? (
